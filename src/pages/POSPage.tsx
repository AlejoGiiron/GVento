...
import {
  Search, X, ChevronDown, Plus, Trash, Minus, ShoppingCart, Percent,
...
import type { ProductWithCategory, CartItem } from '@/stores/cartStore'
import type { Enums } from '@/types/database.types'

type OrderType = 'dine_in' | 'takeaway' | 'delivery'
type PaymentMethodUI = 'efectivo' | 'tarjeta' | 'nequi'
...
function CartPanel({
  ...
  const orderTypes: { id: OrderType; label: string; icon: React.ReactNode; bg: string; fg: string }[] = [
    { id: 'dine_in',  label: 'Mesa',       icon: <Utensils size={17} />, bg: '#ecfdf5', fg: '#065f46' },
    { id: 'takeaway', label: 'Para llevar', icon: <Store size={17} />,    bg: '#fef3c7', fg: '#854d0e' },
    { id: 'delivery', label: 'Delivery',    icon: <Bike size={17} />,     bg: '#dbeafe', fg: '#1e40af' },
  ]
  ...
  return (
    ...
    >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div
              ...
              onClick={() => {
                ...
                const next = ids[(ids.indexOf(orderType) + 1) % ids.length]
                setOrderType(next)
              }}
      ...
      {/* Discount */}
      <div style={{ padding: '12px 22px 4px', borderTop: '1px solid #f1f5f9' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <Percent size={14} color="#334155" />
          <span style={{ fontSize: 13, color: '#334155', fontWeight: 500 }}>Descuento</span>
          <div style={{ flex: 1 }} />
      ...
      <div style={{ padding: '12px 22px', borderTop: '1px solid #f1f5f9' }}>
        ...
        {discount > 0 && (
          <TotalRow label={`Descuento (${discount}%)`} value={-discountAmt} color="#dc2626" />
        )}
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          >
            Cobrar <ChevronRight size={16} strokeWidth={2.5} />
          </button>
        </div>
      </div>
    </div>
            ...
            <div style={{ padding: 22 }}>
              ...
              <div
                style={{
                  ...
                  padding: '12px 14px', background: change >= 0 ? '#ecfdf5' : '#fef2f2',
        ...
        {step === 'success' && (
          <div style={{ padding: '36px 28px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 13, color: '#64748b', marginBottom: 20 }}>
              {formatCOP(total)} ·{' '}
              {method === 'efectivo' ? 'Efectivo' : method === 'tarjeta' ? 'Tarjeta' : 'Nequi / QR'}
            </div>
            <button
              onClick={onComplete}
              style={{
                padding: '10px 28px', border: 'none', background: '#10b981',
        ...
        <div style={{ padding: '16px 24px 8px', display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: '#0f172a', letterSpacing: -0.3 }}>
              {query ? `Búsqueda: "${query}"` : (activeCatObj?.name ?? 'Todos')}
            </div>
